...
(function () {
   ...
   */
  const createSiteContainerTemplate = () => {
    return (
   ...
   */
  const menuMainElement = runMainElement.querySelector(`.trip-main__trip-controls`);

  if (runMainElement) {
    render(runMainElement, createSiteContainerTemplate());
  }
  ...
  // -? как поставить после h2 ?
  if (menuMainElement) {
    render(menuMainElement, createSiteMenuTemplate());
  }
  if (menuMainElement) {
    render(menuMainElement, createSiteFiltrTemplate(), 'beforeend');
  }
  /**
   * trip-events
   */
  ...
  if (sortMainElement) {
    render(sortMainElement, createSiteContentTemplate(), 'beforeend')
  }
